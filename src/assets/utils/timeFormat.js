export default {
    ...
    getWeekthInMonth(date) {
        ...
        const preMonth  = new Date(date.getFullYear(), date.getMonth(), 0);
        let week = preMonth.getDay();   //上个月最后一天周几
        // week += 1;    //加上这段则是从周日开始，否则从周一开始
        week = week === 7 ? 0:week;     //本月一号在一周中连着上个月多少天，从周日开始算
        let days = date.getDate();  //当前是几号
        days += week;  //用于计算完整周期内是第几天
        const weekth = Math.ceil((days)/7);
        return weekth;
    },
    getDateInfo(date) {
        date = this.convertToDate(date);
        ...
        let weekStr = "";
        const monthStr = this._processtime(month);
        const dayStr = this._processtime(day);
        switch(week){
    ...
    timestampToTimeStr(timestamp) {
        ...
        try {
            ...
            const nowDateInfo = this.getDateInfo(now);
    ...
    getPreMonth(date) {
        const dateInfo = this.getDateInfo(date);
        const preMonth = new Date(dateInfo.year, dateInfo.month - 1, 0);
        return preMonth;
    },
    getDaysInMonth(date) {
        //返回当前月份总天数
    ...
    convertStrToDate(str) {
        ...
        const yearmonthday = str_arr[0];
        const hourminsecond = str_arr[1] || "";
        const hourminsecond_arr = hourminsecond.split(":");
        let yearmonthday_arr = yearmonthday.split("-");
